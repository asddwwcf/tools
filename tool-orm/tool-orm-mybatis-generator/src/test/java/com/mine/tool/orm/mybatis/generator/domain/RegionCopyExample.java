...
public class RegionCopyExample implements Example<RegionCopy> {
    ...
    public String getCondition() {
        String tableName = TableUtils.getTableName(RegionCopy.class);
        String columnMapping = TableUtils.getColumnMapping(RegionCopy.class);
        StringBuilder sql = new StringBuilder("select ").append(columnMapping).append(" from ").append(tableName);
        StringBuilder condition = new StringBuilder();
        for (RegionCopyExample.Criteria criteria : this.oredCriteria){
            List<RegionCopyExample.Criterion> criterions = criteria.getAllCriteria();
            if(null==criterions||criterions.isEmpty()){ continue; }
            int index = this.oredCriteria.indexOf(criteria);
            StringBuilder conditions = new StringBuilder();
            for (RegionCopyExample.Criterion criterion : criterions){
    ...
    public static class Criteria extends GeneratedCriteria {
        private RegionCopyExample example;

        protected Criteria(RegionCopyExample example) {
            super();
            this.example = example;
        }

        public RegionCopyExample example() {
            return this.example;
        }
    }
